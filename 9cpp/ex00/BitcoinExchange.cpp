...
{
    if (this != &exchange)
        this->exchange_rates = exchange.exchange_rates;
    return (*this);
}
...
{    
    if (date > H_DATE)
        return (exchange_rates.find(H_DATE)->second);
...
std::map<std::string, float> BitcoinExchange::parseDataFile(const std::string &arg) const
{
    ...
    std::getline(file, line); // skip header from data file
    while (std::getline(file, line))
    { // gets one line at a time from file
        ...
        {
            std::cerr << "Error: Invalid line in data file: " << line << std::endl;
            return (temp);
        }
